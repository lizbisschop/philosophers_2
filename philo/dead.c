...
/*   Updated: 2022/06/17 11:51:42 by lbisscho      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
bool check_dead(t_philosopher *philo)
{
    ...
    if (philo->tab->dead_bool == true)
    {
        ...
        if (pthread_mutex_unlock(&philo->tab->dead_mutex))
...
void *dead(void* d)
{
    ...
    while (true)
    {
        while (i < data->total_philos)
        {
            ...
            time = get_time_now();
            if (time - data->philos[i].last_time_eaten > data->philos[i].time_die && (data->philos[i].times_to_eat == true && data->philos[i].times_to_eat != data->philos[i].times_eaten))
            {
